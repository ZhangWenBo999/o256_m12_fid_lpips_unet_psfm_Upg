...
class CCSABlock(nn.Module):
    def __init__(self, 
        ...
        if self.channel_att:
            ...
            # CGLU(in_features=768, hidden_features=512, out_features=768)
            self.c_attention = nn.ModuleList([CGLU(in_features=feature, hidden_features=512, out_features=feature) for feature, head in zip(features, channel_head)])
    ...
    def m_apply(self, x, module):
        ...
        for i, j in enumerate(x):
            B, C, H, W = j.shape
            j_3d = to_3d(j)
            output.append(module[i](j_3d, H, W))
        return output

      # for i, j in enumerate(x):
      #     B, C, H, W = j.shape
      #     j_3d = to_3d(j)
      #     return module[i](j_3d, H, W)
